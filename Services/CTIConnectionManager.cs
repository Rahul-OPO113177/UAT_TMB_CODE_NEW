    ...
    public static class CTIConnectionManager
    {
        ...
        public static bool LoginAgent(CL_AgentDet agentvalue, string agentId, string dn, string tServerIp, string tServerPort, string Location, string OPOID, string ProcessName, out string errorMessage)
        {
            ...
            try
            {
                ...
                catch (Exception ex)
                {
                    CTIConnectionManager.LogToFile(
                        "Exception while opening TServer: " + ex.Message + " | Stack: " + ex.StackTrace,
                        OPOID
                    );
                    return false;
                }
                    ...
                    if (regResponse.Name == "EventRegistered")
                    {
                        ...
                        if (loginResponse.Name == "EventAgentLogin")
                        {
                            ...
                            if (readyResponse.Name == "EventAgentReady")
                            {
                                ...
                                try
                                {
                                    InfoPageFeilds.WarmUpMySqlConnection();
                                    CTIConnectionManager.LogToFile(" WarmUpMySqlConnection Done", OPOID);

                                }
                                catch (Exception ex)
                                {
                                    CTIConnectionManager.LogToFile(" WarmUpMySqlConnection Faile" + ex.Message, OPOID);
                                }

                                try
                                {
                                    InfoPageFeilds.getConnectionstring(agentvalue.ProcessName);
                                    CTIConnectionManager.LogToFile(" Dynamic Done" , OPOID);

                                }
                                catch (Exception ex)
                                {
                                    CTIConnectionManager.LogToFile(" Dynamic Faile" + ex.Message, OPOID);
                                }
            ...
            catch (Exception ex)
            {
                ...
                CTIConnectionManager.LogToFile("Error While open Connection  --" + tServerIp + "-" + tServerPort + "--" + agentvalue.login_code, OPOID);
                CTIConnectionManager.LogToFile(ex.Message, OPOID);
            ...
            lock (session.LockObj)
            {
                switch (msg.Name)
                {
                    ...
                    case EventNetworkReached.MessageName:
                        ...
                        {
                            session.Call_Type = Convert.ToString(eventNetworkReached.CallType);
                           
                            session.ConnID = eventNetworkReached.ConnID;

                            if (session.Call_Type == "Outbound")
                            {
                                session.CampaignPhone = eventNetworkReached.DNIS;
                            }

                            if (session.CallUuid == null)
                            {
                                session.CallUuid = eventNetworkReached.CallUuid;
                            }
                    ...
                    case EventEstablished.MessageName:
                        ...
                        session.Call_Type = Convert.ToString(established.CallType);
                        ...
                        if (established.ThisDN.ToString().Equals(session.DN))
                        {
                            ...
                            if (session.CallUuid == null)
                            {
                                session.CallUuid = established.CallUuid;
                            }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (established.UserData.Keys[i] == "GSW_CHAIN_ID")
                                    {
                                        session.MyCode = Convert.ToDouble(established.UserData["GSW_CHAIN_ID"].ToString());
                                    }
                        ...
                        if (released.ThisDN == session.DN)
                        {
                            ...
                            else
                            {
                                if (session.IVRConnID != null)
                                {
                                    ...
                                    session.ConnID = releasecall.ConnID;
                    ...
                    case EventAttachedDataChanged.MessageName:
                        ...
                        if (eventattacheddatachanged.ThisDN == session.DN)
                        {
                            ...
                            for (int i = 0; i < eventattacheddatachanged.UserData.Count; i++)
                            {
                                ...
                                {
                                    session.attempts = int.Parse(eventattacheddatachanged.UserData["GSW_ATTEMPTS"].ToString());
                                }
                                ...
                                {
                                    session.MyCode = Convert.ToDouble(eventattacheddatachanged.UserData["TMasterID"].ToString());
                                }
                        ...
                        if (eventUserEvent.ThisDN == session.DN)
                        {
                            ...
                            if (eventUserEvent.UserData.GetAsString("GSW_USER_EVENT") != null)
                            {
                                ...
                                if (eventUserEvent.UserData["GSW_USER_EVENT"].ToString() == "ScheduledCall")
                                {
                                    ...
                                    if (session.CurrentStatusID == 1 && session.PCBMyCode > 0)
                                    {
                                        ...
                                        if (session.MyCode > 0)
                                        {
                                            session.CampaignPhone = Convert.ToString(eventUserEvent.UserData["GSW_PHONE"].ToString());
                                            session.MasterPhone = Convert.ToString(eventUserEvent.UserData["GSW_PHONE"].ToString());
                                            session.lblcallback = "Personal Callback";
                                    ...
                                    {
                                        CTIConnectionManager.Show_Data(session.MyCode, session.AgentId);
                                ...
                                if (eventUserEvent.UserData.GetAsString("GSW_ERROR") != null)
                                {
                                    ...
                                    {
                                        session.CurrentStatusID = 1;
                                        if (session.ProcessType == "OUTBOUND")
                                        {
                                            session.MyCode = CTIConnectionManager.GetPCB(session.AgentId);
                                            if (session.MyCode > 0)
                                            {
                                                if (session.CampaignMode != "Preview")
                                                {
        ...
        public static async Task GetInfoPageFeilds(string ProcessName, string AgnetID, string Mycode, string PhoneNumer)
        {
            ...
            var Registeredcustno = InfoPageFeilds.RegisteredCustomer(PhoneNumer);
            await CTIConnectionManager.HubContext.Clients.Group(AgnetID).SendAsync("Registeredcustno", Registeredcustno);
            await  CTIConnectionManager.HubContext.Clients.Group(AgnetID).SendAsync("infopagedata", infoPageFeilds);
            await CTIConnectionManager.HubContext.Clients.Group(AgnetID).SendAsync("history", history);
            OracleSearchResponse? ifraemAPiResponse = await InfoPageFeilds.SearchOracleByPhone(PhoneNumer);
            string entyti = JsonConvert.SerializeObject(ifraemAPiResponse);
            CTIConnectionManager.LogToFile("ifraemAPiResponse API Is Consumned --  " + entyti, PhoneNumer.ToString());
            await CTIConnectionManager.HubContext.Clients.Group(AgnetID).SendAsync("IframeEntity", ifraemAPiResponse);

        }


        public static async Task SearchOraclePhone(string login_code,string phone)
        {
            AgentSession session = GetAgentSession(login_code);

            OracleSearchResponse? ifraemAPiResponse =
                await InfoPageFeilds.SearchOracleByPhone(phone);

            string entyti = JsonConvert.SerializeObject(ifraemAPiResponse);

            CTIConnectionManager.LogToFile(
                "ifraemAPiResponse API Is Consumned --  " + entyti,
                phone.ToString()
            );

            await CTIConnectionManager.HubContext
                .Clients
                .Group(session.AgentId)
                .SendAsync("IframeEntity", ifraemAPiResponse);
        }
        ...
        private static double GetPCB(string agentID)
        {
            try
            {
                ...
                string connectionString = InfoPageFeilds.getConnectionstring(session.ProcessName);
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    conn.Open();

                    using (MySqlCommand cmd = new MySqlCommand("USP_InsertRetriveFromPCB", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.AddWithValue("p_Mycode", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Phone", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Agent_Name", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Campaign", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_FTIME", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Connid", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Dispo_code", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_Subdispo_code", DBNull.Value);
                        cmd.Parameters.AddWithValue("p_ProcessName", session.ProcessName);
                        cmd.Parameters.AddWithValue("p_Operation", "Select");
                        cmd.Parameters.AddWithValue("p_Opocode", session.OPOID);
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                if (reader.Read())
                                {
                                    ...
                                    double myCode = reader.IsDBNull(0) ? 0 : reader.GetDouble(0);
                                    string phone = reader.IsDBNull(1) ? null : reader.GetString(1);


                                    session.MyCode = myCode;
                                    session.CampaignPhone = phone;
        ...
        private static void Show_Data(double mycode, string AgentID)
        {
            try
            {
                AgentSession session = GetAgentSession(AgentID);
        ...
        public static async Task<string> MakeCall(string exten, string agentId, string phoneNumber)
        {
            ...
            if (session.DialAccess == "0" && session.GetnextDialAccess != "1")
            {
        ...
        //public static void ClearFeilds(AgentSession session)
        //{
            ...
            try
            {
             ...
             //   ClearFeilds(session);
                ...
                else
                {
                    if (session.ProcessType == "OUTBOUND")
                    {
                        session.MyCode = GetPCB(session.AgentId);
                        if (session.MyCode > 0)
                        {
                            if (session.CampaignMode != "Preview")
                            {
                            ...
                            Show_Data(session.MyCode, session.AgentId);
                            session.lblcallback = "Personal Callback";
        ...
        public static string DisposeCall(string logincode, int dispo, int sub_dispo, string cbtime, string finishCode)
        {
            ...
            if (status == 4)
            {
                ...
                if (session.lblcallback == "Personal Callback")
                {
        ...
        private static void InsertCallLogApi(string AgentID)
        {
            try
            {
                ...
                string con = InfoPageFeilds.getConnectionstring(session.ProcessName);
                using (MySqlConnection conn = new MySqlConnection(con))
                {
                    conn.Open();
                    using (MySqlCommand cmd = new MySqlCommand("Usp_Insert_Call_Log", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.AddWithValue("@p_Phonenumber", session.CampaignPhone ?? "");
                        cmd.Parameters.AddWithValue("@p_DialTime", session.StartTime ?? "");
                        cmd.Parameters.AddWithValue("@p_ConnectTime", session.StartTime ?? "");
                        cmd.Parameters.AddWithValue("@p_DisconnectTime", session.EndTime ?? "");
                        cmd.Parameters.AddWithValue("@p_DisconnectType", session.distype ?? "");
                        cmd.Parameters.AddWithValue("@p_AgentID", session.OPOID ?? "");
                        cmd.Parameters.AddWithValue("@p_Process", session.ProcessName ?? "");
                        cmd.Parameters.AddWithValue("@p_Location", session.Location ?? "");
                        cmd.Parameters.AddWithValue("@p_TconnID", session.ConnID?.ToString() ?? "");
                        cmd.Parameters.AddWithValue("@p_CampaignName", session.CampaignName ?? "");
                        cmd.Parameters.AddWithValue("@p_CampaignMode", session.CampaignMode ?? "");
                        cmd.Parameters.AddWithValue("@p_agentgroup", session.AgentGroup ?? "");
                        cmd.Parameters.AddWithValue("@p_VoiceFilePath", (session.RecordingPath ?? "").Replace("\\", "/"));
                        cmd.Parameters.AddWithValue("@p_Mycode", session.MyCode);
                        cmd.Parameters.AddWithValue("@p_Call_Type", session.Call_Type);
                        cmd.Parameters.AddWithValue("@p_CallUuid", session.CallUuid);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Error inserting call log: " + ex.Message);
            }
        }
        ...
        private static void InsertPCB(string callbackdatetime, string AgentID, int dispo, int subdispo)
        {
            try
            {
                AgentSession session = GetAgentSession(AgentID);
                ...
                using (MySqlConnection conn = new MySqlConnection(con))
                {
                    string query = @"INSERT INTO PCB 
                            (Mycode, Phone, Agent_Name, Campaign, FTIME, Connid, Dispo_code, Subdispo_code, ProcessName, Opocode)
                             VALUES (@Mycode, @Phone, @Agent_Name, @Campaign, @FTIME, @Connid, @Dispo_code, @Subdispo_code, @ProcessName, @Opocode)";

                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@Mycode", session.MyCode);
                        cmd.Parameters.AddWithValue("@Phone", session.CampaignPhone);
                        cmd.Parameters.AddWithValue("@Agent_Name", session.OPOID);
                        cmd.Parameters.AddWithValue("@Campaign", session.CampaignName);
                        cmd.Parameters.AddWithValue("@FTIME", callbackdatetime);
                        cmd.Parameters.AddWithValue("@Connid", session.ConnID.ToString());
                        cmd.Parameters.AddWithValue("@Dispo_code", dispo);
                        cmd.Parameters.AddWithValue("@Subdispo_code", subdispo);
                        cmd.Parameters.AddWithValue("@ProcessName", session.ProcessName);
                        cmd.Parameters.AddWithValue("@Opocode", session.OPOID);
        ...
        public static async Task<string> AgentReady(string agentId)
        {
            ...
            else
            {
                ...
                {
                    return "Agent, while on call, please submit the call first and then check.";
                }

                //if (session.CurrentStatusID == 4 || session.CurrentStatusID == 3 || session.CurrentStatusID == 2 || session.CurrentStatusID == 10)
                //{
                //    return "Agent, while wrapping the call, please submit the call first and then check.";
                //}
        ...
        public static void LogToFile(string text, string AgentID)
        {
           ...
           //string logFilePath = "/var/log/servercrm/" + AgentID + "_ServerCRM_Log.txt";
            try
            {
        ...
        public static void StopAutoWrap()
        {
